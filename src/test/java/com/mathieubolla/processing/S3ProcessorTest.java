...
public class S3ProcessorTest {
	...
	public void shouldAcceptDeleteCommands() {
		...
		when(mockS3Scanner.listObjects("bucket")).thenReturn(
				asList(mockS3ObjectSummary));

		when(mockUploadConfiguration.isClearBucketBeforeUpload()).thenReturn(
				true);
	...
	public void shouldAcceptUploadCommands() {
		...
		when(mockDirectoryScanner.scanRegularFiles(mockDirectory)).thenReturn(
				asList(mockFile));
		when(mockUploadConfiguration.getBaseDirectory()).thenReturn(
				mockDirectory);
		when(mockUploadConfiguration.uploadUnitFor(mockFile, mockDate))
				.thenReturn(mockUploadUnit);
